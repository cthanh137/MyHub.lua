...
function Elements:AddDropdown(name, initialOptions, isMulti, callback, infoData)
    ...
    -- HÀM REFRESH CẬP NHẬT DANH SÁCH (ĐÃ FIX)
    local function Refresh(newList)
        options = newList or {}
        
        -- 1. Xóa toàn bộ nút cũ
        ...
        -- 3. Cập nhật lại vùng cuộn
        ListFrame.CanvasSize = UDim2.new(0, 0, 0, #options * itemHeight)
        
        -- 4. Nếu đang mở thì phải tính lại chiều cao khung ngay lập tức
        if dropped then
            local dynamicHeight = math.min(#options * itemHeight, listHeightLimit)
            ListFrame.Size = UDim2.new(1, -20, 0, dynamicHeight)
            DropFrame.Size = UDim2.new(1, 0, 0, 82 + dynamicHeight)
        end
        
        -- 5. Cập nhật lại text hiển thị (trong trường hợp giá trị cũ không còn trong list mới)
        UpdateText()
    end
