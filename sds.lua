...
local function GetIcon(icon, source)
    if not icon or icon == "" then return "" end -- Nếu không có icon thì trả về chuỗi rỗng thay vì nil
    ...
    elseif source == "Lucide" then
        ...
        if icon ~= nil and IconModule[source] then
            ...
            return sourceicon[icon] or "" -- Thêm "or" để tránh nil
        else
...
local lastFriendCheck = 0
		...
		local function checkFriends()
              -- Chỉ cho phép kiểm tra sau mỗi 60 giây
    if tick() - lastFriendCheck > 60 then 
        lastFriendCheck = tick()
    ...
    Description = "Xóa tệp tự động tải",
    Callback = function()
        local path = Luna.Folder .. "/settings/autoload.txt"
        if isfile(path) then
            delfile(path)
            loadlabel:Set({ Text = "None" })
            Luna:Notification({Title = "Hệ thống", Content = "Đã xóa file Autoload", Icon = "check_circle"})
        else
            Luna:Notification({Title = "Hệ thống", Content = "Không tìm thấy file Autoload", Icon = "warning"})
        end
    end,
})
