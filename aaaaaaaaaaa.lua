...
function Library:CreateWindow(options)
    ...
    local function internalSet(v)
        ...
        for _, btn in ipairs(optionButtons) do
            ...
            local fillFrame = btn:FindFirstChild("FillFrame")
            if fillFrame then
                Tween(fillFrame, {Size = isSel and UDim2.new(1, 0, 1, 0) or UDim2.new(0, 0, 1, 0)}, 0.2)
            end
        end
        if callback then callback(selected) end
    end

    -- Hàm Refresh để cập nhật list mới
    local function Refresh(newOptions)
        ...
        for _, opt in pairs(options) do
            local isInitialSelected = false
            if isMulti then
                isInitialSelected = table.find(selected, opt) ~= nil
            else
                isInitialSelected = (selected == opt)
            end
        ...
        -- Nếu đang mở thì cập nhật lại chiều cao khung
        if dropped then
    ...
    -- Đưa Refresh vào Config để có thể gọi từ bên ngoài
