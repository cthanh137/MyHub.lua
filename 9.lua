...
function Library:CreateWindow(options)
    ...
    local function internalSet(v)
        ...
        for _, btn in ipairs(optionButtons) do
            ...
            local check = btn:FindFirstChild("CheckFrame")
            if check then
                Tween(check, {Size = isSel and UDim2.new(1, 0, 1, 0) or UDim2.new(0, 0, 1, 0)}, 0.2)
            end
        end
        if callback then callback(selected) end
    end

    -- HÀM SETVALUES: Dùng để cập nhật danh sách mới từ bên ngoài
    local function SetValues(newList)
        ...
        -- Nếu đang mở thì cập nhật lại chiều cao khung Dropdown ngay lập tức
        if dropped then
    ...
    -- QUAN TRỌNG: Trả về object để biến bên ngoài có thể gọi SetValues
    local dropdownLogic = {
        SetValues = SetValues,
        Set = internalSet,
        Get = function() return selected end
    }
    Window.ConfigElements[name] = dropdownLogic
