...
function Elements:AddDropdown(name, initialOptions, isMulti, callback, infoData)
    ...
    local function toggleDrop(open)
        ...
        if dropped then
            ...
            local dynamicHeight = math.min(DList.AbsoluteContentSize.Y, listHeightLimit)
    ...
    -- Hàm cập nhật danh sách options
    local function SetValues(newList)
        ...
        -- Đợi 1 frame để UIListLayout tính toán lại
        task.spawn(function()
            task.wait()
