...
function Elements:AddDropdown(name, initialOptions, isMulti, callback, infoData)
    local options = initialOptions or {} -- Biến lưu trữ danh sách hiện tại
    local selected = isMulti and {} or (options[1] or nil)
    local dropped = false
    local optionButtons = {}
    local maxVisible = 3
    local itemHeight = 25
    local listHeightLimit = maxVisible * itemHeight
    ...
    local ListFrame = Create("ScrollingFrame", {Parent = DropFrame, BackgroundColor3 = BackgroundColor, Size = UDim2.new(1, -20, 0, 0), Position = UDim2.new(0, 10, 0, 78), CanvasSize = UDim2.new(0, 0, 0, 0), ScrollBarThickness = 2, ScrollBarImageColor3 = Color3.fromRGB(80, 80, 85), BorderSizePixel = 0})
    ...
    -- HÀM REFRESH CỰC KỲ QUAN TRỌNG
    local function Refresh(newList)
        -- 1. Cập nhật bảng options mới
        options = newList or {}
        
        -- 2. Xóa sạch nút cũ trong list
        for _, btn in pairs(optionButtons) do
            btn:Destroy()
        end
        table.clear(optionButtons)

        -- 3. Tạo lại các nút mới
        for _, opt in ipairs(options) do
            local OptBtn = Create("TextButton", {
                ...
                TextSize = 12, 
                TextColor3 = (not isMulti and selected == opt) and TextColor or SubTextColor, 
                BackgroundTransparency = 1, 
                Size = UDim2.new(1, 0, 0, itemHeight), 
                AutoButtonColor = false,
                Visible = true -- Đảm bảo nút luôn hiện
            })
        ...
        -- 4. Ép buộc tính toán lại kích thước hiển thị
        local totalHeight = #options * itemHeight
        ListFrame.CanvasSize = UDim2.new(0, 0, 0, totalHeight)
        
        -- Nếu đang mở mà refresh, phải update lại chiều cao DropFrame ngay lập tức
        if dropped then
            local dynamicHeight = math.min(totalHeight, listHeightLimit)
            ListFrame.Size = UDim2.new(1, -20, 0, dynamicHeight)
            Tween(DropFrame, {Size = UDim2.new(1, 0, 0, 82 + dynamicHeight)}, 0.2)
        end
    end
    ...
    local dropLogic = { SetValues = Refresh, Set = internalSet, Get = function() return selected end }
    Window.ConfigElements[name] = dropLogic
    return dropLogic -- Trả về object để dùng bên ngoài
end
