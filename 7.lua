...
function Elements:AddDropdown(name, initialOptions, isMulti, callback, infoData)
    ...
    local function Refresh(newList)
        ...
        -- Cập nhật vùng cuộn và kích thước
        local totalItemsHeight = #options * itemHeight
        ListFrame.CanvasSize = UDim2.new(0, 0, 0, totalItemsHeight)
        
        if dropped then
            local dynamicHeight = math.min(totalItemsHeight, listHeightLimit)
    ...
    local dropLogic = { SetValues = Refresh, Get = function() return selected end }
    Window.ConfigElements[name] = dropLogic
    return dropLogic -- RẤT QUAN TRỌNG: TRẢ VỀ ĐỂ DÙNG BÊN NGOÀI
end
