...
function Library:CreateWindow(options)
    ...
    local ListFrame = Create("ScrollingFrame", {Parent = DropFrame, BackgroundColor3 = BackgroundColor, Size = UDim2.new(1, -20, 0, listHeight), Position = UDim2.new(0, 10, 0, 78), CanvasSize = UDim2.new(0, 0, 0, 0), ScrollBarThickness = 2, ScrollBarImageColor3 = Color3.fromRGB(80, 80, 85), BorderSizePixel = 0})
    ...
    local function internalSet(v)
        ...
        for _, btn in ipairs(optionButtons) do
            ...
            if btn:FindFirstChild("Fill") then
                Tween(btn.Fill, {Size = isSel and UDim2.new(1, 0, 1, 0) or UDim2.new(0, 0, 1, 0)}, 0.2)
            end
        end
        if callback then callback(selected) end
    end

    -- HÀM REFRESH CẬP NHẬT LIST
    local function Refresh(newOptions)
        ...
        for _, opt in pairs(options) do
            ...
            local Check = Create("Frame", {Name = "Fill", Parent = OptBtn, BackgroundColor3 = AccentColor, Size = isInitialSelected and UDim2.new(1, 0, 1, 0) or UDim2.new(0, 0, 1, 0), BackgroundTransparency = 0.8, BorderSizePixel = 0})
        ...
        -- Nếu đang mở thì cập nhật lại chiều cao khung Dropdown
        if dropped then
            local dynamicHeight = math.min(#options * 25, listHeight)
    ...
    -- QUAN TRỌNG: Trả về Object để có thể gọi từ bên ngoài
    ...
    return DropdownObject -- Dòng này giúp biến WeaponDropdown không bị nil
end
