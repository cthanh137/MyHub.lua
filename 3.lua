...
function Elements:AddDropdown(name, initialOptions, isMulti, callback, infoData)
    ...
    local maxVisible = 4
    local itemHeight = 25
    local listMaxHeight = maxVisible * itemHeight
    ...
    -- HÀM QUAN TRỌNG: Cập nhật lại danh sách mới
    local function SetValues(newList)
        options = newList or {}
        -- Xóa nút cũ hoàn toàn
        for _, btn in ipairs(optionButtons) do btn:Destroy() end
        table.clear(optionButtons)
        
        -- Tạo nút mới từ list mới
        ...
        -- Cập nhật lại Canvas (Vùng cuộn)
        ListFrame.CanvasSize = UDim2.new(0, 0, 0, #options * itemHeight)
        
        -- Nếu đang mở mà bấm Refresh, cập nhật luôn kích thước khung đang hiện
        if dropped then
            local currentHeight = math.min(#options * itemHeight, listMaxHeight)
            ListFrame.Size = UDim2.new(1, -20, 0, currentHeight)
            DropFrame.Size = UDim2.new(1, 0, 0, 82 + currentHeight)
        end
    end
    ...
    MainBtn.MouseButton1Click:Connect(function()
        ...
        if dropped then
            ...
            Tween(ListFrame, {Size = UDim2.new(1, -20, 0, currentHeight)}, 0.3)
            Tween(DropFrame, {Size = UDim2.new(1, 0, 0, 82 + currentHeight)}, 0.3)
        else
            Tween(Arrow, {Rotation = 0}, 0.3)
            Tween(ListFrame, {Size = UDim2.new(1, -20, 0, 0)}, 0.3)
            Tween(DropFrame, {Size = UDim2.new(1, 0, 0, 50)}, 0.3)
            task.delay(0.3, function() if not dropped then ListFrame.Visible = false; SearchBox.Visible = false end end)
        end
    end)
    ...
    SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        for _, btn in ipairs(optionButtons) do
            btn.Visible = (q == "" or string.find(btn.Text:lower(), q)) and true or false
        end
    end)
    ...
    local logic = { SetValues = SetValues, Get = function() return selected end }
    Window.ConfigElements[name] = logic
    return logic -- Trả về để biến bên ngoài gọi được SetValues
end
