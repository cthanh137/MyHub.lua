...
function Luna:CreateWindow(WindowSettings)
	...
	function Tab:BuildConfigSection()
		...
		-- FIX 7: RefreshConfigList xử lý mọi dạng path từ listfiles()
		function Luna:RefreshConfigList()
			...
			local out = {}
			for i = 1, #list do
				-- FIX: listfiles có thể trả về table lồng nhau trên một số executor
				local raw = list[i]
				local file = type(raw) == "table" and tostring(raw[1] or "") or tostring(raw)
				-- Thử match với separator trước
				local name = file:match("[/\\]([^/\\]+)%.luna$")
				-- Nếu không có separator (một số executor trả về tên file thẳng)
				if not name then
					name = file:match("^([^/\\]+)%.luna$")
				end
