...
function Library:CreateWindow(options)
  ...
  function Elements:AddDropdown(name, initialOptions, isMulti, callback, infoData)
    ...
    local closedHeight = 50
    local openBaseHeight = 78

    local DropFrame = Create("Frame", {Parent = ItemContainer, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, closedHeight), ClipsDescendants = true})
    ...
    local ListFrame = Create("ScrollingFrame", {Parent = DropFrame, BackgroundColor3 = BackgroundColor, Size = UDim2.new(1, -20, 0, 0), Position = UDim2.new(0, 10, 0, 50), CanvasSize = UDim2.new(0, 0, 0, 0), ScrollBarThickness = 2, ScrollBarImageColor3 = Color3.fromRGB(80, 80, 85), BorderSizePixel = 0})
    ...
    local function UpdateText()
        if isMulti then
            ...
            MainBtn.Text = txt == "" and "Select Options..." or txt
        else
    ...
    local function internalSet(v)
        ...
        for _, btn in ipairs(optionButtons) do
            ...
            local checkFrame = btn:FindFirstChild("CheckFrame")
            if checkFrame then
                Tween(checkFrame, {Size = isSel and UDim2.new(1, 0, 1, 0) or UDim2.new(0, 0, 1, 0)}, 0.2)
            end
        end
    ...
    local function toggleDrop(open)
        ...
        if dropped then
            ...
            Tween(DropFrame, {Size = UDim2.new(1, 0, 0, openBaseHeight + dynamicHeight)}, 0.3)
        else
            SearchBox.Visible = false
            SearchBox.Text = ""
            ListFrame.Position = UDim2.new(0, 10, 0, 50)
            Tween(Arrow, {Rotation = 0}, 0.3)
            Tween(DropFrame, {Size = UDim2.new(1, 0, 0, closedHeight)}, 0.3)
        end
    end
    ...
    local function SetValues(newList)
        ...
        for _, opt in ipairs(options) do
            ...
            local OptBtn = Create("TextButton", {
                ...
                BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, itemHeight), AutoButtonColor = false
            })
            local Check = Create("Frame", {
                Name = "CheckFrame", Parent = OptBtn, BackgroundColor3 = AccentColor, 
        ...
        local totalHeight = #options * itemHeight
        ListFrame.CanvasSize = UDim2.new(0, 0, 0, totalHeight)
        ListFrame.Size = UDim2.new(1, -20, 0, math.min(totalHeight, listHeightLimit))
        
        if callback then callback(selected) end
    end

    MainBtn.MouseButton1Click:Connect(function()
        toggleDrop(not dropped)
    end)

    SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
        local q = SearchBox.Text:lower()
        for _, btn in ipairs(optionButtons) do
            btn.Visible = (q == "" or string.find(btn.Text:lower(), q, 1, true)) and true or false
        end
    end)
