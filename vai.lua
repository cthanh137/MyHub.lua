	...
	function Tab:BuildConfigSection()
			...
			Tab:CreateInput({
				...
				Callback = function(input)
					-- FIX: đảm bảo inputPath luôn là string
					if type(input) == "table" then
						inputPath = input[1]
					else
						inputPath = tostring(input or "")
					end
				end,
			})
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					if not success then
						...
						return -- FIX 1: thêm return để không chạy tiếp khi lỗi
					end
			...
			configSelection = Tab:CreateDropdown({
				...
				SpecialType = nil,
				Callback = function(Value)
					-- FIX: Dropdown callback trả về table, cần lấy [1]
			...
			Tab:CreateButton({
				...
				Callback = function()
					-- FIX 2: kiểm tra selectedConfig nil
			...
			Tab:CreateButton({
				...
				Callback = function()
					-- FIX 3: kiểm tra selectedConfig nil
					if not selectedConfig then
						Luna:Notification({
							Title = "Interface",
							Icon = "warning",
			...
			Tab:CreateButton({
				...
				Callback = function()
					-- FIX 4: kiểm tra selectedConfig nil
			...
			["Colorpicker"] = {
				...
				Load = function(Flag, data)
					...
					if Luna.Options[Flag] and data.color then
						-- FIX 5: thêm Alpha khi load Colorpicker
						Luna.Options[Flag]:Set({
							Color = HexToColor3(data.color),
							Alpha = data.alpha
						})
					end
				end
		...
		function Tab:BuildThemeSection()
			...
			-- FIX 6: thay task.wait(1) bằng RunService.Heartbeat:Wait() đáng tin cậy hơn
			game:GetService("RunService").Heartbeat:Wait()

			c1cp:Set({
				Callback = function(Value)
					if c2cp and c3cp then
						local col1 = (type(Value) == "userdata" and Value) or Color3.fromRGB(255,255,255)
						local col2 = (type(c2cp.Color) == "userdata" and c2cp.Color) or Color3.fromRGB(255,255,255)
						local col3 = (type(c3cp.Color) == "userdata" and c3cp.Color) or Color3.fromRGB(255,255,255)
						Luna.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, col1), ColorSequenceKeypoint.new(0.50, col2), ColorSequenceKeypoint.new(1.00, col3)}
			...
			c2cp:Set({
				Callback = function(Value)
					if c1cp and c3cp then
						local col1 = (type(c1cp.Color) == "userdata" and c1cp.Color) or Color3.fromRGB(255,255,255)
						local col2 = (type(Value) == "userdata" and Value) or Color3.fromRGB(255,255,255)
						local col3 = (type(c3cp.Color) == "userdata" and c3cp.Color) or Color3.fromRGB(255,255,255)
						Luna.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, col1), ColorSequenceKeypoint.new(0.50, col2), ColorSequenceKeypoint.new(1.00, col3)}
						LunaUI.ThemeRemote.Value = not LunaUI.ThemeRemote.Value
					end
			...
			c3cp:Set({
				Callback = function(Valuex)
					if c2cp and c1cp then
						local col1 = (type(c1cp.Color) == "userdata" and c1cp.Color) or Color3.fromRGB(255,255,255)
						local col2 = (type(c2cp.Color) == "userdata" and c2cp.Color) or Color3.fromRGB(255,255,255)
						local col3 = (type(Valuex) == "userdata" and Valuex) or Color3.fromRGB(255,255,255)
						Luna.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, col1), ColorSequenceKeypoint.new(0.50, col2), ColorSequenceKeypoint.new(1.00, col3)}
						LunaUI.ThemeRemote.Value = not LunaUI.ThemeRemote.Value
					end
				end
		...
		-- FIX 7: RefreshConfigList an toàn trên mọi môi trường
		function Luna:RefreshConfigList()
			if isStudio then return {} end
			if not listfiles then return {} end

			local ok, list = pcall(listfiles, Luna.Folder .. "/settings")
			if not ok or type(list) ~= "table" then return {} end

			local out = {}
			for i = 1, #list do
				local raw = list[i]
				if raw == nil then continue end
				-- listfiles có thể trả về table lồng hoặc string tuỳ executor
				local file = type(raw) == "table" and tostring(raw[1] or "") or tostring(raw)
				if file == "" or file == "nil" then continue end
				-- Match tên file bỏ extension .luna
				local name = file:match("[/\\]([^/\\]+)%.luna$") or file:match("^([^/\\]+)%.luna$")
				if name and name ~= "options" and name ~= "" then
