...
function Library:CreateWindow(options)
    ...
    local function internalSet(v)
        ...
        for _, btn in ipairs(optionButtons) do
            local isSel = isMulti and table.find(selected, btn.Text) or (selected == btn.Text)
    ...
    local function Refresh(newOptions)
        -- Kiểm tra nếu newOptions rỗng
        options = newOptions or {}
        print("Dropdown Refreshing: Found " .. #options .. " items") -- KIỂM TRA TRONG F9
        ...
        for _, opt in pairs(options) do
            local isInitialSelected = isMulti and table.find(selected, opt) or (selected == opt)
            ...
            OptBtn.MouseButton1Click:Connect(function()
                if isMulti then
                    ...
                    internalSet(opt)
                    MainBtn.MouseButton1Click:Fire() -- Đóng list khi chọn
                end
            end)
        end
        
        -- Cập nhật Canvas và Reset Search
        ListFrame.CanvasSize = UDim2.new(0, 0, 0, #options * itemHeight)
        SearchBox.Text = "" 
        
        -- Cập nhật lại chiều cao nếu đang mở
        if dropped then
            local dynamicHeight = math.min(#options * itemHeight, listMaxHeight)
    ...
    local logic = { SetValues = Refresh, Set = internalSet, Get = function() return selected end }
