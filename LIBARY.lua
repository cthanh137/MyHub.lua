    ...
    function Window:CreateTab(tabName, isDefault, isLocked)
        ...
        function TabConfig:CreatePage(pageName)
            ...
            function Elements:AddConfigManager(folderName)
                    ...
                    local autoLoadFile = folderName .. "/autoload.txt" -- File ghi nhớ config tự động
                    ...
                    local function LoadData(filepath, displayFName)
                        ...
                        if s and type(data) == "table" then
                            ...
                            Library:Notify({Title = "Auto-Load", Description = "Successfully loaded config: " .. displayFName})
                        end
                    end
                    ...
                    local function RefreshMonitor()
                        ...
                        -- Kiểm tra xem file nào đang là Auto-Load
                        local currentAuto = ""
                        ...
                        for _, filepath in ipairs(files) do
                            ...
                            if rawName then
                                ...
                                -- Nút LOAD
                                local LoadBtn = Create("TextButton", {Parent = Row, Text = "L", Font = Enum.Font.GothamBold, TextSize = 10, TextColor3 = TextColor, BackgroundColor3 = Color3.fromRGB(45, 120, 60), Size = UDim2.new(0, 25, 0, 20), Position = UDim2.new(1, -100, 0.5, -10)})
                                Create("UICorner", {Parent = LoadBtn, CornerRadius = UDim.new(0, 4)})
                                
                                -- Nút AUTO (Mới)
                                local AutoBtn = Create("TextButton", {Parent = Row, Text = "Auto", Font = Enum.Font.GothamBold, TextSize = 9, TextColor3 = TextColor, BackgroundColor3 = isAuto and AccentColor or Color3.fromRGB(60, 60, 70), Size = UDim2.new(0, 35, 0, 20), Position = UDim2.new(1, -72, 0.5, -10)})
                                Create("UICorner", {Parent = AutoBtn, CornerRadius = UDim.new(0, 4)})

                                -- Nút EDIT
                                local EditBtn = Create("TextButton", {Parent = Row, Text = "E", Font = Enum.Font.GothamBold, TextSize = 10, TextColor3 = TextColor, BackgroundColor3 = Color3.fromRGB(150, 100, 45), Size = UDim2.new(0, 25, 0, 20), Position = UDim2.new(1, -33, 0.5, -10)})
                                Create("UICorner", {Parent = EditBtn, CornerRadius = UDim.new(0, 4)})

                                LoadBtn.MouseButton1Click:Connect(function()
                                    if deleteMode or editMode then return end
                                    LoadData(filepath, displayFName)
                                end)

                                AutoBtn.MouseButton1Click:Connect(function()
                                    if isAuto then
                                        _writefile(autoLoadFile, "") -- Hủy auto load
                                        Library:Notify({Title = "Auto-Load", Description = "Disabled auto-load."})
                                    else
                                        _writefile(autoLoadFile, filepath) -- Ghi đè đường dẫn file auto load
                                        Library:Notify({Title = "Auto-Load", Description = displayFName .. " will load on script run."})
                                    end
                                    RefreshMonitor()
                                end)
                    ...
                    -- Chạy Auto-Load khi khởi tạo xong Manager
                    task.spawn(function()
                        task.wait(0.5) -- Đợi các element UI khởi tạo xong hoàn toàn
                        if isfile(autoLoadFile) then
                            local path = _readfile(autoLoadFile)
                            if path ~= "" and isfile(path) then
                                local name = path:match("([^/\\]+)%.json$") or "Config"
                                LoadData(path, name)
                            end
                        end
                    end)
                    ...
                    -- (Các nút Delete, Cancel giữ nguyên logic cũ của bạn...)
