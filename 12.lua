    ...
    function Window:CreateTab(tabName, isDefault, isLocked)
        ...
        function TabConfig:CreatePage(pageName)
         ...
         function Elements:AddDropdown(name, initialOptions, isMulti, callback, infoData)
                    ...
                    -- HÀM REFRESH QUAN TRỌNG NHẤT ĐỂ CẬP NHẬT LIST MỚI
                    local function SetValues(newList)
                        options = newList or {}
                        
                        -- Xóa sạch các nút cũ trong UI và trong bảng
                        for _, btn in ipairs(optionButtons) do btn:Destroy() end
                        table.clear(optionButtons)

                        -- Tạo lại các nút dựa trên danh sách mới
                        for _, opt in ipairs(options) do
                            local isInitialSelected = isMulti and table.find(selected, opt) or (selected == opt)
                            local OptBtn = Create("TextButton", {Parent = ListFrame, Text = opt, Font = Enum.Font.Gotham, TextSize = 12, TextColor3 = isInitialSelected and TextColor or SubTextColor, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, itemHeight), AutoButtonColor = false})
                            local Check = Create("Frame", {Name = "CheckFrame", Parent = OptBtn, BackgroundColor3 = AccentColor, Size = isInitialSelected and UDim2.new(1, 0, 1, 0) or UDim2.new(0, 0, 1, 0), BackgroundTransparency = 0.8})
                        ...
                        -- Cập nhật CanvasSize dựa trên số lượng item mới
                        ListFrame.CanvasSize = UDim2.new(0, 0, 0, #options * itemHeight)
                        
                        -- Nếu đang mở Dropdown thì phải cập nhật lại chiều cao khung ngay lập tức
                        if dropped then
                            local dynamicHeight = math.min(#options * itemHeight, listHeightLimit)
                            ListFrame.Size = UDim2.new(1, -20, 0, dynamicHeight)
                            Tween(DropFrame, {Size = UDim2.new(1, 0, 0, 82 + dynamicHeight)}, 0.1)
                        end
                        UpdateText()
                    end
                    ...
                    -- TRẢ VỀ OBJECT ĐỂ SỬ DỤNG BÊN NGOÀI
                    local dropLogic = {
                        SetValues = SetValues,
                        Set = internalSet,
                        ...
                        SetValues = function(newOptions)
                            ...
                            for _, opt in pairs(options) do
                                ...
                                local Check = Create("Frame", {
                                    Parent = OptBtn, BackgroundColor3 = AccentColor, 
