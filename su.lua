...
local function GetIcon(icon, source)
	if not icon then
		return "rbxassetid://6031097225" -- Material Icon "menu" làm fallback
	end
	...
	elseif source == "Lucide" then
		...
		return "rbxassetid://6031097225" -- Fallback nếu có lỗi
	else	
		if icon ~= nil and IconModule[source] then
			...
			if result then
				...
				-- Thử tìm trong Material icons với tên khác
				for name, id in pairs(IconModule.Material) do
					if string.lower(name) == string.lower(icon) then
						return id
					end
				end
				return "rbxassetid://6031097225" -- Fallback
...
function Luna:Notification(data) -- action e.g open messages
	task.spawn(function()
		...
		local iconAsset = GetIcon(data.Icon, data.ImageSource)
		if iconAsset then
			newNotification.Icon.Image = iconAsset
		else
			-- Fallback icon nếu không tìm thấy
			newNotification.Icon.Image = "rbxassetid://6031097225" -- Material Icon "menu"
			newNotification.Icon.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
			newNotification.Icon.BackgroundTransparency = 0
			newNotification.Icon.Size = UDim2.new(0, 20, 0, 20)
		end
...
function Luna:CreateWindow(WindowSettings)
	...
	-- Sử dụng Luna.Folder thay vì hardcode
local keyFolder = Luna.Folder .. "/Key System/"
EnsureFolder(keyFolder)  -- Tạo thư mục nếu chưa có

local keyFilePath = keyFolder .. WindowSettings.KeySettings.FileName .. ".luna"
		f isfile and isfile(keyFilePath) then
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					...
					if WindowSettings.KeySettings.SaveKey then
						if writefile then
						writefile(keyFilePath, FoundKey)
	...
	function Window:CreateHomeTab(HomeTabSettings)
        ...
        local function checkFriends()
            if friendsCooldown == 0 then
                friendsCooldown = 60  -- Tăng lên 60 giây
                ...
                -- Bọc trong pcall để tránh lỗi rate limit
                local success = pcall(function()
                    local list = Players:GetFriendsAsync(Player.UserId)
                    local pages = 0
                    while true and pages < 3 do  -- Giới hạn tối đa 3 trang
                        pages = pages + 1
                ...
                if not success then
                    HomeTabPage.detailsholder.dashboard.Friends.All.Value.Text = "Error loading"
                    HomeTabPage.detailsholder.dashboard.Friends.Offline.Value.Text = "Error"
                    HomeTabPage.detailsholder.dashboard.Friends.Online.Value.Text = "Error"
                    HomeTabPage.detailsholder.dashboard.Friends.InGame.Value.Text = "Error"
                    return
                end

                -- Lấy danh sách bạn online
                local onlineSuccess, onlineData = pcall(function()
                    return Player:GetFriendsOnline()
                end)

                if onlineSuccess and onlineData then
                    for i, v in pairs(onlineData) do
                        if v then
                ...
                for i, v in pairs(playersFriends) do
                    if v and v.Username and Players:FindFirstChild(v.Username) then
		...
		function Tab:BuildConfigSection()
		   ...
		   Tab:CreateInput({
                ...
                Callback = function(input)
                    -- Đảm bảo input là string
                    if type(input) == "table" then
                        inputPath = input[1] or ""
                    else
                        inputPath = input or ""
                    end
                end,
			...
			local configSelection
                ...
                Callback = function()
                    -- Đảm bảo inputPath là string
                    local configName = inputPath
                    if type(configName) == "table" then
                        configName = configName[1] or ""
                    end
                    
                    if not configName or type(configName) ~= "string" or string.gsub(configName, " ", "") == "" then
                        Luna:Notification({
                            Title = "Interface",
                            Icon = "warning",
                            ImageSource = "Material",
                    ...
                    if not Luna.SaveConfig then
                        Luna:Notification({
                            Title = "Interface",
                            Icon = "error",
                            ImageSource = "Material",
                            Content = "SaveConfig method not available. Please check your Luna library version."
                        })
                        return
                    end
                    ...
                    Luna:Notification({
                        ...
                        Icon = "info",
                        ImageSource = "Material",
                        Content = string.format("Created config %q", configName),
                    })
  ...
  Tab:CreateButton({
                ...
                Callback = function()
                    if not Luna.LoadConfig then
                        Luna:Notification({
                            Title = "Interface",
                            Icon = "error",
                            ImageSource = "Material",
                            Content = "LoadConfig method not available. Please check your Luna library version."
                        })
                        return
                    end

                    -- Xử lý selectedConfig nếu là table (từ dropdown)
                    ...
                    local success, returned = Luna:LoadConfig(configName)
                    if not success then
                        Luna:Notification({
                            Title = "Interface",
                            Icon = "error",
                            ImageSource = "Material",
                            Content = "Unable to load config, return error: " .. tostring(returned)
                        })
                        return
                    end
                    ...
                    Luna:Notification({
                        ...
                        Icon = "info",
                        ImageSource = "Material",
                        Content = string.format("Loaded config %q", configName),
                    })
                end
            })
            ...
            Tab:CreateButton({
                ...
                Callback = function()
                    if not Luna.SaveConfig then
                        Luna:Notification({
                            ...
                            Icon = "error",
                            ImageSource = "Material",
                            Content = "SaveConfig method not available."
                        })
                        return
                    end

                    -- Xử lý selectedConfig nếu là table
                    local configName = selectedConfig
                    if type(configName) == "table" then
                        configName = configName[1] or ""
                    end
              ...
              local success, returned = Luna:SaveConfig(configName)
                    ...
                    Luna:Notification({
                        ...
                        Icon = "info",
                        ImageSource = "Material",
                        Content = string.format("Overwrote config %q", configName),
                    })
                end
            })
		   ...
		   Tab:CreateButton({
			...
			loadlabel = Tab:CreateParagraph({
                ...
                Callback = function()
                    local filePath = Luna.Folder .. "/settings/autoload.txt"
                    if isfile(filePath) then
                        ...
                        Luna:Notification({
                            ...
                            Icon = "warning",
                            ImageSource = "Material",
                            Content = "No autoload file found.",
                        })
                    end
                end,
            })
        ...
        local function SetFolder()
            ...
            local root = WindowSettings.ConfigSettings.RootFolder or ""
            local configFolder = WindowSettings.ConfigSettings.ConfigFolder or "Big Hub"

            if root ~= "" then
                Luna.Folder = root .. "/" .. configFolder
            else
                Luna.Folder = configFolder
            end
        ...
        function Luna:SaveConfig(Path)
            if isStudio then return false, "Config system unavailable." end
        ...
        function Luna:LoadConfig(Path)
            ...
            for _, option in next, decoded.objects do
                -- Kiểm tra tồn tại của option và option.type
                if option and option.type and ClassParser[option.type] then
                    -- Kiểm tra option.flag tồn tại trong Luna.Options
                    if Luna.Options[option.flag] then
                        task.spawn(function() 
