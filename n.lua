...
}-- Rate limit handling cho Friends API
local friendsRateLimit = {
	cooldown = 0,
	maxRequests = 1,
	currentRequests = 0
}

local function CanMakeFriendsRequest()
	if friendsRateLimit.cooldown > 0 then
		return false
	end
	
	if friendsRateLimit.currentRequests >= friendsRateLimit.maxRequests then
		friendsRateLimit.cooldown = 30  -- Chờ 30 giây
		return false
	end
	
	friendsRateLimit.currentRequests = friendsRateLimit.currentRequests + 1
	return true
end

-- Reset cooldown mỗi frame
game:GetService("RunService").Heartbeat:Connect(function()
	if friendsRateLimit.cooldown > 0 then
		friendsRateLimit.cooldown = friendsRateLimit.cooldown - 0.1
		if friendsRateLimit.cooldown < 0 then
			friendsRateLimit.cooldown = 0
			friendsRateLimit.currentRequests = 0
		end
	end
end)
...
function Luna:CreateWindow(WindowSettings)
	...
	if friendsCooldown == 0 then
		if not CanMakeFriendsRequest() then
			HomeTabPage.detailsholder.dashboard.Friends.All.Value.Text = "Rate limited"
			return
		end
		friendsCooldown = 60
		...
		local success, err = pcall(function()
			...
			while true do
				for _, data in list:GetCurrentPage() do
					...
					table.insert(playersFriends, data)  -- Sửa Data thành data
				end
		...
		if not success then
			-- Nếu lỗi, hiển thị thông báo và không cập nhật
                ...
                Callback = function()
                    ...
                    if not success then
                        Luna:Notification({
                            ...
                            Icon = "error",
                            ImageSource = "Material",
                            Content = "Unable to overwrite config, return error: " .. tostring(returned)
                        })
                        return
                    end
