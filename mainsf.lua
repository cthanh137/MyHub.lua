...
local AutoLoadFilePath = "auto_load.txt"
...
function Library:CreateWindow(options)
    -- Lấy tên thư mục config từ options, mặc định là "zyronxSavers"
    local configFolderName = "zyronxSavers"
    if type(options) == "table" and options.ConfigFolder then
        configFolderName = options.ConfigFolder
    end
        ...
        function TabConfig:CreatePage(pageName)
            ...
            function PageObj:CreateSection(sectionName)
                ...
                function Elements:AddConfigManager(folderName)
                    folderName = folderName or Window.ConfigFolder or "zyronxSavers"
                    ...
                    AddInfoIcon(ManagerFrame, UDim2.new(1, -20, 0, -22), {
                        ...
                        "2. Delete Mode: Toggle ON, click files to select, then click 'Delete Selected'.\n" ..
                        "3. Load: Click 'Load' on any save to apply it.\n" ..
                        "4. Edit: Click 'Edit' to rename a save.\n" ..
                        "5. Auto-Load: Click ★ to set this config to auto-load when UI starts."
                    })
                    ...
                    local function RefreshMonitor()
                        ...
                        for _, filepath in ipairs(files) do
                            ...
                            if rawName then
                                ...
                                local baseName = rawName:gsub("_%d+%.%d+$", ""):gsub("_%d+$", "")
                                ...
                                local LoadBtn = Create("TextButton", {Parent = Row, Text = "Load", Font = Enum.Font.GothamBold, TextSize = 10, TextColor3 = TextColor, BackgroundColor3 = Color3.fromRGB(45, 120, 60), Size = UDim2.new(0, 35, 0, 20), Position = UDim2.new(1, -175, 0.5, -10), AutoButtonColor = false})
                                Create("UICorner", {Parent = LoadBtn, CornerRadius = UDim.new(0, 4)})
                                AddBounce(LoadBtn)

                                local EditBtn = Create("TextButton", {Parent = Row, Text = "Edit", Font = Enum.Font.GothamBold, TextSize = 10, TextColor3 = TextColor, BackgroundColor3 = Color3.fromRGB(150, 100, 45), Size = UDim2.new(0, 30, 0, 20), Position = UDim2.new(1, -140, 0.5, -10), AutoButtonColor = false})
                                ...
                                Create("UICorner", {Parent = AutoBtn, CornerRadius = UDim.new(0, 4)})
                                AddBounce(AutoBtn)
                                
                                -- Kiểm tra auto-load
                                local autoFilePath = folderName .. "/" .. AutoLoadFilePath
                                local currentAuto = _readfile(autoFilePath) or ""
                                if currentAuto == baseName then
                                    AutoBtn.BackgroundColor3 = AccentColor
                                    AutoBtn.TextColor3 = Color3.fromRGB(255,255,255)
                                end

                                AutoBtn.MouseButton1Click:Connect(function()
                                    if deleteMode or editMode then return end
                                    local currentAuto2 = _readfile(autoFilePath) or ""
                                    if currentAuto2 == baseName then
                                        _writefile(autoFilePath, "")
                                        AutoBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 80)
                                        AutoBtn.TextColor3 = TextColor
                                        Library:Notify({Title = "Auto-Load", Description = "Đã tắt auto-load cho: " .. displayFName})
                                    else
                                        _writefile(autoFilePath, baseName)
                                        AutoBtn.BackgroundColor3 = AccentColor
                                        AutoBtn.TextColor3 = Color3.fromRGB(255,255,255)
                                        Library:Notify({Title = "Auto-Load", Description = "Đã bật auto-load cho: " .. displayFName})
                                    end
                                    RefreshMonitor()
                                end)
    ...
    -- // AUTO-LOAD CONFIG - Đặt ở cuối CreateWindow
    task.delay(0.8, function()
        local folderPath = configFolderName or "zyronxSavers"
        local autoFilePath = folderPath .. "/" .. AutoLoadFilePath
        
        if not _isfolder(folderPath) then 
            _makefolder(folderPath) 
        end
        
        local autoName = _readfile(autoFilePath)
        if autoName and autoName ~= "" then
            local files = _listfiles(folderPath)
            local targetFile = nil
            for _, f in ipairs(files) do
                local name = f:match("([^/\\]+)%.json$")
                if name then
                    local baseName = name:gsub("_%d+%.%d+$", ""):gsub("_%d+$", "")
            ...
            if targetFile then
                local success, data = pcall(function() 
                    return HttpService:JSONDecode(_readfile(targetFile)) 
                end)
                if success and type(data) == "table" then
                    ...
                    Library:Notify({
                        Title = "🔄 Auto-Load", 
                        Description = "Đã tải config: " .. autoName
                    })
                else
                    warn("[Auto-Load] Không thể parse config: " .. autoName)
                end
            else
                warn("[Auto-Load] Không tìm thấy file cho: " .. autoName)
            end
        end
    end)
