...
function Library:CreateWindow(options)
  ...
  function Elements:AddDropdown(name, initialOptions, isMulti, callback, infoData)
    local dropdownData = {
        options = initialOptions or {},
        selected = isMulti and {} or (initialOptions[1] or nil),
        dropped = false,
        optionButtons = {}
    }
    ...
    local Title = Create("TextLabel", {Parent = DropFrame, Text = name, Font = Enum.Font.Gotham, TextSize = 12, TextColor3 = SubTextColor, BackgroundTransparency = 1, Size = UDim2.new(1, -20, 0, 15), Position = UDim2.new(0, 10, 0, 0), TextXAlignment = Enum.TextXAlignment.Left})
    
    local MainBtn = Create("TextButton", {Parent = DropFrame, Text = "Select...", Font = Enum.Font.Gotham, TextSize = 12, TextColor3 = TextColor, BackgroundColor3 = BackgroundColor, Size = UDim2.new(1, -20, 0, 26), Position = UDim2.new(0, 10, 0, 20), AutoButtonColor = false, TextXAlignment = Enum.TextXAlignment.Left})
    ...
    local ListFrame = Create("ScrollingFrame", {Parent = DropFrame, BackgroundColor3 = BackgroundColor, Size = UDim2.new(1, -20, 0, 0), Position = UDim2.new(0, 10, 0, 78), CanvasSize = UDim2.new(0, 0, 0, 0), ScrollBarThickness = 2, ScrollBarImageColor3 = Color3.fromRGB(80, 80, 85), BorderSizePixel = 0, Visible = false})
    ...
    local function UpdateText()
        if isMulti then
            local txt = ""
            for _, v in pairs(dropdownData.selected) do txt = txt .. v .. ", " end
            MainBtn.Text = txt == "" and "Select Options..." or txt:sub(1, -3)
        else
            MainBtn.Text = dropdownData.selected or "Select..."
        end
    end

    local function Refresh(newList)
        dropdownData.options = newList or {}
        
        -- Xóa nút cũ
        for _, btn in ipairs(dropdownData.optionButtons) do btn:Destroy() end
        table.clear(dropdownData.optionButtons)
        ...
        for _, opt in ipairs(dropdownData.options) do
            ...
            table.insert(dropdownData.optionButtons, OptBtn)
            
            OptBtn.MouseButton1Click:Connect(function()
                if isMulti then
                    local idx = table.find(dropdownData.selected, opt)
                    if idx then table.remove(dropdownData.selected, idx) else table.insert(dropdownData.selected, opt) end
                else
                    dropdownData.selected = opt
                    dropdownData.dropped = false
                    Tween(Arrow, {Rotation = 0}, 0.3)
                    Tween(ListFrame, {Size = UDim2.new(1, -20, 0, 0)}, 0.3)
                    Tween(DropFrame, {Size = UDim2.new(1, 0, 0, 50)}, 0.3)
                    task.delay(0.3, function() ListFrame.Visible = false; SearchBox.Visible = false end)
                end
                UpdateText()
                if callback then callback(dropdownData.selected) end
            end)
        end
        
        -- Cập nhật CanvasSize
        ListFrame.CanvasSize = UDim2.new(0, 0, 0, #dropdownData.options * itemHeight)
        
        -- Nếu đang mở mà refresh thì cập nhật luôn chiều cao
        if dropdownData.dropped then
            local dynamicHeight = math.min(#dropdownData.options * itemHeight, maxVisible * itemHeight)
            ListFrame.Size = UDim2.new(1, -20, 0, dynamicHeight)
            DropFrame.Size = UDim2.new(1, 0, 0, 82 + dynamicHeight)
        end
    end

    MainBtn.MouseButton1Click:Connect(function()
        dropdownData.dropped = not dropdownData.dropped
        if dropdownData.dropped then
            local dynamicHeight = math.min(#dropdownData.options * itemHeight, maxVisible * itemHeight)
            ...
            Tween(ListFrame, {Size = UDim2.new(1, -20, 0, dynamicHeight)}, 0.3)
            Tween(DropFrame, {Size = UDim2.new(1, 0, 0, 82 + dynamicHeight)}, 0.3)
        else
            Tween(Arrow, {Rotation = 0}, 0.3)
            Tween(ListFrame, {Size = UDim2.new(1, -20, 0, 0)}, 0.3)
            Tween(DropFrame, {Size = UDim2.new(1, 0, 0, 50)}, 0.3)
            task.delay(0.3, function() if not dropdownData.dropped then ListFrame.Visible = false; SearchBox.Visible = false end end)
        end
    end)

    Refresh(initialOptions)
    UpdateText()
    
    local logic = { SetValues = Refresh, Get = function() return dropdownData.selected end }
    Window.ConfigElements[name] = logic
    return logic
end
           ...
           function Elements:AddConfigManager(folderName)
                    ...
                    -- Popup xác nhận xóa (Giao diện phụ)
                    local InternalConfirmPopup = Create("Frame", {Parent = ManagerFrame, BackgroundColor3 = Color3.fromRGB(25, 25, 30), Size = UDim2.new(1, 0, 1, 0), Position = UDim2.new(0, 0, 0, 0), ZIndex = 100, Visible = false})
                    Create("UICorner", {Parent = InternalConfirmPopup, CornerRadius = UDim.new(0, 8)})
                    local P_Title = Create("TextLabel", {Parent = InternalConfirmPopup, Text = "Confirm Delete?", Font = Enum.Font.GothamBold, TextSize = 14, TextColor3 = Color3.fromRGB(255, 80, 80), Size = UDim2.new(1, 0, 0, 40), BackgroundTransparency = 1, ZIndex = 101})
                    local P_Yes = Create("TextButton", {Parent = InternalConfirmPopup, Text = "YES", BackgroundColor3 = Color3.fromRGB(200, 50, 50), Size = UDim2.new(0.4, 0, 0, 30), Position = UDim2.new(0.1, 0, 0.6, 0), TextColor3 = TextColor, ZIndex = 101})
                    local P_No = Create("TextButton", {Parent = InternalConfirmPopup, Text = "NO", BackgroundColor3 = Color3.fromRGB(60, 60, 65), Size = UDim2.new(0.4, 0, 0, 30), Position = UDim2.new(0.5, 0, 0.6, 0), TextColor3 = TextColor, ZIndex = 101})
                    Create("UICorner", {Parent = P_Yes, CornerRadius = UDim.new(0, 4)}); Create("UICorner", {Parent = P_No, CornerRadius = UDim.new(0, 4)})

                    local function LoadData(filepath, displayFName)
                        local s, data = pcall(function() return HttpService:JSONDecode(_readfile(filepath)) end)
                        if s and type(data) == "table" then
                            for k, v in pairs(data) do
                                if Window.ConfigElements[k] and Window.ConfigElements[k].Set then Window.ConfigElements[k].Set(v) end
                            end
                            Library:Notify({Title = "Config", Description = "Loaded: " .. displayFName})
                        end
                    end
                    ...
                    local function RefreshMonitor()
                        ...
                        selectedForDelete = {}
                        local currentAuto = ""
                        pcall(function() if isfile(autoLoadFile) then currentAuto = _readfile(autoLoadFile) end end)
                        ...
                        for _, filepath in ipairs(files) do
                            ...
                            if rawName then
                                ...
                                local isAuto = (currentAuto == filepath)
                                ...
                                local Title = Create("TextLabel", {Parent = Row, Text = (isAuto and "⭐ " or "") .. displayFName, Font = Enum.Font.Gotham, TextSize = 11, TextColor3 = isAuto and AccentColor or TextColor, BackgroundTransparency = 1, Size = UDim2.new(1, -110, 1, 0), Position = UDim2.new(0, 10, 0, 0), TextXAlignment = Enum.TextXAlignment.Left})
                                
                                local LoadBtn = Create("TextButton", {Parent = Row, Text = "L", BackgroundColor3 = Color3.fromRGB(45, 120, 60), Size = UDim2.new(0, 25, 0, 20), Position = UDim2.new(1, -100, 0.5, -10), TextColor3 = TextColor})
                                local AutoBtn = Create("TextButton", {Parent = Row, Text = "Auto", BackgroundColor3 = isAuto and AccentColor or Color3.fromRGB(60, 60, 70), Size = UDim2.new(0, 35, 0, 20), Position = UDim2.new(1, -72, 0.5, -10), TextColor3 = TextColor})
                                local EditBtn = Create("TextButton", {Parent = Row, Text = "E", BackgroundColor3 = Color3.fromRGB(150, 100, 45), Size = UDim2.new(0, 25, 0, 20), Position = UDim2.new(1, -33, 0.5, -10), TextColor3 = TextColor})
                                
                                Create("UICorner", {Parent = LoadBtn, CornerRadius = UDim.new(0, 4)}); Create("UICorner", {Parent = AutoBtn, CornerRadius = UDim.new(0, 4)}); Create("UICorner", {Parent = EditBtn, CornerRadius = UDim.new(0, 4)})

                                -- Nút chọn để xóa (phủ lên Row khi ở chế độ xóa)
                                local SelectMask = Create("TextButton", {Parent = Row, Text = "", BackgroundTransparency = 1, Size = UDim2.new(1, -110, 1, 0), ZIndex = 5, Visible = deleteMode})

                                SelectMask.MouseButton1Click:Connect(function()
                                ...
                                LoadBtn.MouseButton1Click:Connect(function() if not deleteMode then LoadData(filepath, displayFName) end end)
                                AutoBtn.MouseButton1Click:Connect(function()
                                    _writefile(autoLoadFile, isAuto and "" or filepath)
                                    RefreshMonitor()
                                end)
                                EditBtn.MouseButton1Click:Connect(function()
                                    editMode = true; editTargetFile = filepath; NameBox.Text = displayFName
                                    ActionArea.Visible = true; CreateBtn.Visible = false; DeleteTogBtn.Visible = false
                                    ConfirmActionBtn.Text = "Save Edit"; ConfirmActionBtn.BackgroundColor3 = Color3.fromRGB(50, 150, 200)
                                end)
                            end
                        end
                    end
                    ...
                    DeleteTogBtn.MouseButton1Click:Connect(function()
                        ...
                        DeleteTogBtn.BackgroundColor3 = deleteMode and Color3.fromRGB(150, 50, 50) or Color3.fromRGB(45, 45, 50)
                        ActionArea.Visible = deleteMode; CreateBtn.Visible = not deleteMode
                        ConfirmActionBtn.Text = "Delete Selected"; ConfirmActionBtn.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
                        RefreshMonitor()
                    end)
                    ...
                    ConfirmActionBtn.MouseButton1Click:Connect(function()
                        ...
                        elseif editMode then
                            ...
                            _writefile(folderName .. "/" .. NameBox.Text .. "_" .. tick() .. ".json", HttpService:JSONEncode(payload))
                            editMode = false; ActionArea.Visible = false; CreateBtn.Visible = true; DeleteTogBtn.Visible = true; RefreshMonitor()
                        end
                    end)

                    P_Yes.MouseButton1Click:Connect(function()
                        for file, _ in pairs(selectedForDelete) do pcall(function() _delfile(file) end) end
                        InternalConfirmPopup.Visible = false; deleteMode = false
                        DeleteTogBtn.Text = "Delete Mode: OFF"; DeleteTogBtn.BackgroundColor3 = Color3.fromRGB(45, 45, 50)
                        ActionArea.Visible = false; CreateBtn.Visible = true; RefreshMonitor()
                    end)

                    P_No.MouseButton1Click:Connect(function() InternalConfirmPopup.Visible = false end)

                    CancelActionBtn.MouseButton1Click:Connect(function()
                        editMode = false; deleteMode = false; ActionArea.Visible = false; CreateBtn.Visible = true; DeleteTogBtn.Visible = true; RefreshMonitor()
                    end)

                    CreateBtn.MouseButton1Click:Connect(function()
                        local payload = {}
                        for k, el in pairs(Window.ConfigElements) do if el.Get then payload[k] = el.Get() end end
                        _writefile(folderName .. "/" .. NameBox.Text .. "_" .. math.floor(tick()) .. ".json", HttpService:JSONEncode(payload))
                        RefreshMonitor()
                    end)

                    -- Auto Load khi vừa mở
                    task.spawn(function()
                        task.wait(0.5)
                        if isfile(autoLoadFile) then
                            local p = _readfile(autoLoadFile)
                            if p ~= "" and isfile(p) then LoadData(p, "Auto-Config") end
                        end
                    end)
