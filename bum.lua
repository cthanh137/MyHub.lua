...
function Luna:CreateWindow(WindowSettings)
	...
	if WindowSettings.KeySystem then
		...
		if dragBar and dragInteract then Draggable(dragInteract, Main, true, 255) end
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		local executorName = pcall(identifyexecutor) and identifyexecutor() or "Your Executor Does Not Support identifyexecutor."
		HomeTabPage.detailsholder.dashboard.Client.Title.Text = (isStudio and "Debugging (Studio)" or executorName)
...
if isStudio then
	-- Comment or remove this section if not needed
end
