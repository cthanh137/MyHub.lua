...
function Library:CreateWindow(options)
    ...
    local openBaseHeight = 78 -- 50 (Title+Btn) + 24 (Search) + 4 (Gap)
    ...
    local ListFrame = Create("ScrollingFrame", {Parent = DropFrame, BackgroundColor3 = BackgroundColor, Size = UDim2.new(1, -20, 0, listHeight), Position = UDim2.new(0, 10, 0, 50), CanvasSize = UDim2.new(0, 0, 0, #options * 25), ScrollBarThickness = 2, ScrollBarImageColor3 = Color3.fromRGB(80, 80, 85), BorderSizePixel = 0})
    ...
    -- Hàm xử lý Animation mở/đóng chuẩn hóa
    local function toggleDrop(open)
        dropped = open
        if dropped then
            SearchBox.Visible = true
            ListFrame.Position = UDim2.new(0, 10, 0, 78) -- Đẩy xuống dưới SearchBox
            ...
            SearchBox.Text = ""
            ListFrame.Position = UDim2.new(0, 10, 0, 50) -- Lùi lên sát MainBtn
    ...
    for _, opt in pairs(options) do
        ...
        OptBtn.MouseButton1Click:Connect(function()
            if isMulti then
                ...
                internalSet(opt)
                toggleDrop(false) -- Đóng dropdown
            end
        end)
    end
    UpdateText()

    -- Tìm kiếm không dùng Pattern (tránh lỗi khi gõ ký tự đặc biệt)
    SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
        local q = SearchBox.Text:lower()
        for _, btn in ipairs(optionButtons) do
            if q == "" or string.find(btn.Text:lower(), q, 1, true) then 
                btn.Visible = true 
            else 
                btn.Visible = false 
            end
    ...
    DList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        ...
        if dropped then
            local dynamicHeight = math.min(DList.AbsoluteContentSize.Y, listHeight)
            ListFrame.Size = UDim2.new(1, -20, 0, dynamicHeight)
            Tween(DropFrame, {Size = UDim2.new(1, 0, 0, openBaseHeight + dynamicHeight)}, 0.1)
        end
    end)
